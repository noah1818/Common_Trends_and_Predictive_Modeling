...
class ClientHYPERLIQUID(threading.Thread):
    """
    WebSocket client wrapper for maintaining a persistent connection
    with automatic reconnection and ping handling.
    ...
    Notes
    -----
    This module defines the base 'ClientHYPERLIQUID' class, which manages persistent
    WebSocket connection mechanisms (including reconnection logic, ping handling, and
    error recovery).

    Specialized client implementations can extend this class to process specific
    data streams. For example:

    - 'hyperliquid_perptual_futures_orderbook.py' implements 'HYPERLIQUIDOrderbookManager', a WebSocket client for
    handling L2 orderbook data in real-time.

    Both this base client and its WebSocket subclasses are implemented according to 
    the official Hyperliquid Exchange API documentation as of 2025-08-25. 
