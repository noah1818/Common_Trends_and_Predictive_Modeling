...
from client_bybit import ClientBYBIT
from utils import get_timestamp_ms, get_monotonic_s

class BYBITOrderbookManager(ClientBYBIT):
    """
    Bybit USDT-Futures L1 order book manager with application-level heartbeats.
    ...
    channel : str
        Bybit channel name, e.g. "orderbook.<level>.<symbol>".
    application_level_ping_interval_s : float
        Interval (seconds) to send application-level '"ping"' (Bybit expects a certain ping frame).
    application_level_pong_timeout_s : float
        Max time (seconds) allowed since last pong before closing the socket.

    Notes
    -----
    - Bybit replies with a pong frame.
    - Protocol-level pings ('ping_interval', 'ping_timeout') are orthogonal and handled
      by the base client. This class manages Bybit's app-level ping/pong.
    """
    ...
    def _ping_loop(self, ws: websocket.WebSocketApp) -> None:
        """
        Send bybit application-level 'ping' or ping frame at a fixed cadence.
        ...
        while not self._hb_stop.is_set() and ws.sock and ws.sock.connected:
            try:
                ws.send(json.dumps({"req_id": "100001", "op": "ping"})) # Bybit expects this ping frame
    ...
    def update_orderbook(self, bids: list[list[str, str]] | list, asks: list[list[str, str]] | list) -> None:
        """
    ...
    def on_message(self, ws: websocket.WebSocketApp, message: bytes) -> None:
        """
        Handles incoming WebSocket messages, L2 data and updates the book of top 5 levels, respectively.
        ...
        try:
            if isinstance(message, bytes):
                ...
                if msg.get('op') == 'subscribe':
                    # Bybit sends subscription confirmation messages
                    return

                elif msg.get('ret_msg') == 'pong':
                    ...
                    self._orderbook = {'bids': {}, 'asks': {}}
                    data_bids = msg.get('data').get('b')
                    data_asks = msg.get('data').get('a')
    ...
    def on_open(self, ws: websocket.WebSocketApp) -> None:
        ...
        ws : websocket.WebSocketApp
            Active WebSocket connection.
        """
        sub_msg = {"op": "subscribe", "args": [f"{self._channel}.1.{self._symbol}"]}
