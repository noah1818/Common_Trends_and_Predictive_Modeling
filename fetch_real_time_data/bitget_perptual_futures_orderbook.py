...
from client_bitget import ClientBITGET
from utils import get_timestamp_ms, get_monotonic_s

class BITGETOrderbookManager(ClientBITGET):
    """
    Bitget USDT-Futures L1 order book manager with application-level heartbeats.
    ...
    def _ping_loop(self, ws: websocket.WebSocketApp) -> None:
        """
        Send Bitget application-level 'ping' at a fixed cadence.
        ...
        while not self._hb_stop.is_set() and ws.sock and ws.sock.connected:
            try:
                ws.send("ping") # Bitget expects plain string
        ...
        try:
            if isinstance(message, bytes):
                if message == b"pong":
                    ...
                    if msg.get('event', None) == 'subscribe':
                        # Bitget sends subscription confirmation messages
                        return

                    elif msg.get('arg', {'channel': None}).get('channel') == self._channel:
                        # Initialize sequence number
                        ts = int(msg.get('data')[0].get('ts'))
                        bid_price = float(msg.get('data')[0].get('bids')[0][0])
                        bid_size = float(msg.get('data')[0].get('bids')[0][1])
                        ask_price = float(msg.get('data')[0].get('asks')[0][0])
                        ask_size = float(msg.get('data')[0].get('asks')[0][1])
    ...
    def on_open(self, ws: websocket.WebSocketApp) -> None:
        """
        Subscribe to the books1 channel and start heartbeats.
